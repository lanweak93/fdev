...
function paramautfill
{
	...
	if [ ! ${msvcstatustext} ];then
		...
		do
			msvcstatustext[${yy}]="curl --fail --silent http://127.0.0.1:${port[${yy}]}/getstatus"
		done
	...
	if [ ! ${msvcdowntext} ];then
		for ((yy=0;yy<${container_num};yy++))
		do
			msvcdowntext[${yy}]="curl --fail --silent http://127.0.0.1:${port[${yy}]}/down"
		done
		msvcdowntext_num=0
	else
	...
	if [ ! ${msvcdeletetext} ];then
		for ((yy=0;yy<${container_num};yy++))
		do
			msvcdeletetext[${yy}]="curl --fail --silent http://127.0.0.1:${port[${yy}]}/delete"
		done
	...
	if [ ! ${msvcuptext} ];then
		for ((yy=0;yy<${container_num};yy++))
		do
			msvcuptext[${yy}]="curl --fail --silent http://127.0.0.1:${port[${yy}]}/up"
		done
		msvcuptext_num=0
	else
